...
export function FeedbackForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      console.log("[Feedback]", address ? `Polkadot address: ${address}\n\n` : "", feedback.trim());

      const res = await fetch("/api/feedback", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ feedback: feedback.trim(), address: address ?? undefined }),
      });
    ...
    <div className={styles.feedbackContainer}>
      <div className={styles.feedbackSection}>
        ...
        <p className={styles.rewardInfo}>
          <span className={styles.highlight}>Constructive feedback will be rewarded in-game.</span>
